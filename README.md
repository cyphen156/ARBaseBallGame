...
야구 게임 로직과 시스템 전반을 담당하고, 아래 세 시스템을 직접 설계·구현했습니다.

- **[입력 파이프라인](#입력-파이프라인)** : 드래그 조작과 UI 입력을 게임에서 사용하는 값과 명령으로 변환해 전달하는 프로세스입니다.
  - 드래그 입력 정규화
  - 규칙 기반 UI 입력 이벤트 처리
- **[마그누스 효과 기반 투구 물리](#마그누스-효과-기반-투구-물리)** : 공의 회전과 중력 방향을 조합해 네 방향의 커브를 만드는 시스템입니다.
- **[충돌 기반 타격 물리 시스템](#충돌-기반-타격-물리-시스템)** : 충돌 위치와 스윙 시점으로 전달 힘을 근사하고, 공의 기존 상태를 반영해 타구를 만드는 시스템입니다.
...
애니메이션에 종속된 배트의 물리 속도 대신 충돌 위치와 스윙 경과 시간으로 전달 힘을 근사하고, 공의 기존 진행 방향과 속력을 반영해 타구를 계산하는 시스템입니다.
...
| 처리 | 기준 |
|---|---|
| 충돌 위치 | 공과 충돌한 Collider의 배열 인덱스 |
| 위치 가중치 | 첫·마지막 인덱스를 `0~1`로 정규화해 `0.3~1.3` 범위로 변환 |

- 분할 Collider가 실제 접촉 영역과 회전축에서의 거리를 구분
- 회전축에서 먼 Collider일수록 높은 위치 가중치 적용

#### 배트의 회전

드래그의 세로 성분으로 스윙 시작 시 배트 기울기를 정하고, 애니메이션 경과 시간을 전달 힘의 가중치로 사용하는 기능입니다.

| 처리 | 기준 |
|---|---|
| 스윙 경로 | 캐릭터의 스윙 애니메이션 |
| 배트 기울기 | 드래그 세로 성분을 `±22.5°` 범위로 제한 |
| 스윙 시점 | 애니메이션 시작 후 경과 시간 |

- 입력 방향은 스윙 시작 시 확정되며 진행 중에는 변경하지 않음
- 애니메이션에 종속된 배트의 물리 속도 대신 스윙 경과 시간을 사용

#### 타구의 생성

입력으로 정한 배트 방향을 기준으로 공의 기존 진행 방향을 반사하고, 충돌 시점의 전달 힘과 공의 기존 속력을 결합해 최종 타구를 결정하는 기능입니다.

| 결과 | 처리 |
|---|---|
| 전달 힘 | 입력 세기 × 위치 가중치 × 스윙 경과 시간 |
| 타구 방향 | 배트 방향을 기준으로 공의 기존 진행 방향을 반사 |
| 타구 속력 | 전달 힘 + 공의 기존 속력 |
