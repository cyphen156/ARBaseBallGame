...
이를 보완하기 위해 중력 가속도를 조정하고 아래 방향 커브 볼은 중력을 역방향으로 보정했으며,  
magnusStrength를 스케일링하여 **회전 효과가 명확히 드러나도록 설계**했습니다. 
...
입력 정규화는 화면 공간 기준으로는 성공했지만, AR 경기장 배치 시점에 결정되는 카메라-스트라이크존 간 **월드 거리가 force 계산에 반영되지 않았습니다.**  
결과적으로 경기장을 가까이 배치하면 공이 너무 빠르게, 멀리 배치하면 너무 약하게 느껴지는 체감 편차가 발생했습니다.  
이 문제는 마그누스 튜닝과 연결됩니다.  
force 값이 절대적인 기준을 갖지 못하게 되면서 `magnusStrength`를 고정값으로 강제 보정하는 방식에 의존하게 되었고,  
결과적으로 투구의 강약에 관계없이 동일한 횡력이 적용되어 **강속구일수록 더 날카롭게 휘는 물리적 역동성을 구현하지 못했습니다.** 
입력 시스템과 물리 시스템 모두 동일한 원인(월드 거리 미반영)에서 비롯된 보정 구조를 가지게 된 셈입니다.  

개선 방향은 두 가지입니다.
 
- **방법 A — 월드 스케일 정규화** : AR Raycast 거리를 기준으로 월드 스케일 자체를 보정 → force 및 magnusStrength 동시 정규화
- **방법 B — 동적 파라미터 산출** : 카메라-스트라이크존 거리를 force 보정값으로 반영 → magnusStrength를 force에 비례해 동적 산출, 강한 투구일수록 더 깊게 꺾이는 물리적 일관성 확보
