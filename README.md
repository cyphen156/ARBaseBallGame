...
드래그 입력과 버튼 입력을 각각 목적에 맞는 명령으로 변환해 전달하는 입력 처리 구조입니다.

- 드래그 입력은 화면 비율 기준으로 정규화해 물리력으로 변환합니다.
- 해상도와 종횡비에 관계없이 동일한 입력 결과가 나오도록 보정합니다.
- 버튼 입력은 단일 이벤트로 수집해 규칙 기반 명령으로 변환합니다.
- 변환된 입력은 각 처리 주체(Ball, Bat, UI 등)로 전달됩니다.

![입력 파이프라인](./docs/gifs/입력%20파이프라인.png)

#### 드래그 입력 정규화

드래그 입력을 기기 해상도와 화면 비율에 관계없이 동일한 물리력으로 변환합니다.

- 드래그를 해상도로 나눠 정규화된 값으로 변환
- 종횡비 보정을 적용해 수직 · 수평 입력 기준 통일
- 동일 비율 · 방향의 입력 → 항상 동일한 물리력 생성
...
버튼 입력을 규칙 기반 명령으로 변환해 동일한 방식으로 처리하는 입력 구조입니다.

- 모든 버튼 입력을 단일 클릭 이벤트에서 처리합니다.
- 버튼 이름을 기준으로 PlayMode 또는 Command 타입으로 변환합니다.
- 변환된 명령은 UIManager를 통해 각 처리 주체로 전달됩니다.
- 버튼마다 별도의 처리 함수나 스크립트 없이 동일한 규칙으로 확장할 수 있습니다.
